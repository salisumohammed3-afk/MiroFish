      ...
      <Transition name="modal">
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <!-- Non-replayable steps hint -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step 3 "Run Simulation" and Step 5 "Deep Interaction" must be started during a live run and cannot be replayed</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Actual height: rows * card height + (rows-1) * gap + small bottom margin
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Expand downward, add spacing from the title
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // No action needed if target state matches current state
...
.grid-pattern {
  ...
  /* Anchored to top-left; height changes only extend at the bottom without shifting existing grid */
