...
// Reset all state (for restarting the simulation)
const resetAllState = () => {
  phase.value = 0
  runStatus.value = {}
  allActions.value = []
...
// Track each platform's previous round to detect changes and log them
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes for each platform and log them
...
const checkPlatformsCompleted = (data) => {
  // If there's no platform data, return false
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Don't auto-scroll; let the user freely browse the timeline
