            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but the entire section may not be finished yet) -->
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*(?:Select|Choose)\s+([^(]+)(?:[(]index\s*=?\s*\d+[)])?[:/]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        if (name) {
          ...
          // Reason continuation line (exclude concluding summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // Special handling if first question is preceded by "1."
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward compatible: when only one platform is marked)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      const qPattern = /(?:^|[\r\n]+)Question\s*(\d+):\s*/gi
      while ((match = qPattern.exec(answerText)) !== null) {
        matches.push({
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 heading (## xxx), since section title is already shown in the outer container
  ...
  // Clean up <br> before paragraph opening tags that immediately follow block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists: maintain incrementing numbers when single-item <ol>s are separated by paragraph content
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled uniformly in the nextTick after the loop ends
          }
...
const extractFinalContent = (response) => {
  ...
  // Legacy fallback: try alternate "Final Answer" format with colon variants
  const altFinalMatch = response.match(/Final\s+Answer\s*[:]\s*\n*([\s\S]*)$/i)
  if (altFinalMatch) {
