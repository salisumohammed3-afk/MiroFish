...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // ensuring all edges distribute in the same reference frame without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational pull toward center so isolated node clusters stay centered
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from right side of node and loop back
    ...
    // Perpendicular offset proportional to distance, ensuring the curve is clearly visible
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // Reset previously selected edge styling
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('end', (event, d) => {
        // Only let the simulation wind down if a real drag occurred
        if (d._isDragging) {
