...
# This fixes the issue where OASIS third-party libraries read files
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout/stderr streams to UTF-8 (fixes garbled console output)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This fixes the issue where third-party libraries (e.g., OASIS) read files
        without specifying encoding.
        """
...
# Script is located in the backend/scripts/ directory.
...
# Load .env file from project root directory (contains LLM_API_KEY and other config)
...
# Add the filter immediately at module load time to ensure it takes effect before camel code executes
logging.getLogger().addFilter(MaxTokensWarningFilter())


def disable_oasis_logging():
    """
    Disable verbose OASIS library logging output.
    OASIS logs are too verbose (logging every agent's observations and actions);
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize logging config for the simulation
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows showing real entity names in actions.jsonl instead of
    codenames like "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last read max rowid value (using rowid instead of created_at because
                     different platforms use different created_at formats)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions, each containing agent_id, agent_name, action_type,
                        action_args (with context info)
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM configuration for speed boost during parallel simulation:
    ...
    for different platforms to increase concurrency.
    ...
    # Fall back to config if model name not in .env
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    # Use OASIS default names for agents not in config
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: simulation doesn't exit after completion,
    waits for interview commands. When a termination signal is received:
