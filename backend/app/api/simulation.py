...
def prepare_simulation():
    ...
    Features:
    - Auto-detects completed preparation work to avoid duplicate generation
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if the simulation is running or completed, it will stop and clean up run logs
    ...
    try:
        ...
        # Smart status handling: if preparation work is complete, allow restart
        if state.status != SimulationStatus.READY:
            # Check if preparation work is complete
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation work is complete, check if a process is running
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions shows only the current latest round content from both platforms
...
def interview_agents_batch():
    ...
    try:
        ...
        # Optimize each interview item's prompt, add prefix to prevent Agent tool calls
