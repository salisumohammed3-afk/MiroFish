...
2. Plan the outline structure first, then generate each section
...
class ReportLogger:
    ...
    def log_section_full_complete(
        ...
        The frontend should listen for this log to determine when a section is truly complete and get full content
        """
        self.log(
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports," with a "God's-eye view" of the simulation world — you can see every Agent's behavior, statements, and interactions.

[Core Concept]
We have built a simulation world and injected a specific "simulation requirement" as a variable. The evolution results of the simulation world represent predictions of what may happen in the future. You are observing not "experimental data," but a "rehearsal of the future."
...
1. Under our specified conditions, what state does the future present?
...
2. [Must quote Agent original behaviors and statements]
   - Agent statements and behaviors are predictions of future human behavior
   - Use quotation format to display these predictions in the report, for example:
...
- PROHIBITED: Using any Markdown headings (#, ##, ###, ####, etc.) within a section
- PROHIBITED: Adding the section main heading at the beginning of content
- Section titles are automatically added by the system; you only need to write body text
- USE bold text (**bold**), paragraph breaks, quotes, and lists to organize content, but do not use headings
...
Weibo, as the front line of public opinion, served as the core channel for initial information release:
...
- panorama_search: Wide-angle panoramic search, understand the full picture, timeline, and evolution process
...
Strictly prohibited:
- Including both tool calls and Final Answer in a single response
...
3. Use Markdown format (but headings are prohibited):
   ...
   - PROHIBITED: Using #, ##, ###, #### or any heading syntax
...
7. [Re-emphasis] Do not add any headings! Use **bold** instead of subsection headings"""
...
- Write body text directly, using **bold** instead of subsection headings
...
class ReportAgent:
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        2. Bare JSON (response is entirely or a single line is a tool call JSON)
        """
        tool_calls = []
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            # Accept this content as the final answer directly
    ...
    def generate_report(
        ...
        try:
            ...
            # Phase 2: Generate sections one by one (save each section separately)
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Starting line to read from (for incremental retrieval, 0 means from the beginning)
